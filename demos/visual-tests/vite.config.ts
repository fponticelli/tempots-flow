...
function approvalApiPlugin() {
  return {
    ...
    configureServer(server: { middlewares: { use: Function } }) {
      ...
      server.middlewares.use(
        '/api/approve',
        (req: { method?: string; url?: string }, res: { statusCode?: number; end: Function }) => {
          if (req.method !== 'POST') {
            res.statusCode = 405
            res.end()
            return
          ...
          const currentPath = path.resolve(__dirname, `../../e2e/current/${scenarioId}.png`)
          const baselinePath = path.resolve(__dirname, `../../e2e/baselines/${scenarioId}.png`)
              ...
              try {
                ...
                if (Array.isArray(results)) {
                  const entry = results.find(
                    (r: { scenarioId?: string }) => r.scenarioId === scenarioId,
                  )
                  if (entry) {
                    entry.status = 'pass'
      ...
      server.middlewares.use(
        ...
        (
          ...
          const filePath = path.resolve(__dirname, '../../e2e', req.url?.slice(1) ?? '')
